...
class tensorOptimisation:
    ...
    def vectors_personal_best(self):
        """Move towards each particle's personal best position."""
        personal_vector = np.zeros_like(self.particles)
        
        for i, particle in enumerate(self.particles):
            # Get personal best from particle history
            if hasattr(self, 'particle_personal_bests'):
                p_best_node = self.particle_personal_bests[i, 0]
                p_best_params = self.particle_personal_bests[i, 1:]
                
                # Node: move along shortest path to personal best
                all_paths = list(nx.all_shortest_paths(
                    self.graph, 
                    source=int(particle[0]), 
                    target=int(p_best_node)
                ))
                
                if len(all_paths) == 1:
                    if len(all_paths[0]) > 1:
                        personal_vector[i, 0] = all_paths[0][1]
                    else:
                        personal_vector[i, 0] = p_best_node
                else:
                    second_elements = [path[1] for path in all_paths]
                    personal_vector[i, 0] = np.random.choice(second_elements)
                
                # Parameters: move towards personal best
                distance = p_best_params - particle[1:]
                personal_vector[i, 1:] = particle[1:] + self.learning_rate * distance
            else:
                personal_vector[i] = particle
        
        return personal_vector

    def total_vector_standard(self, w_inertia=0.33, w_personal=0.33, w_global=0.34):
        """
        Standard PSO update rule with three components:
        - Random/inertia (exploration)
        - Personal best (exploitation of individual experience)
        - Global best (exploitation of swarm knowledge)
        
        Weights should sum to 1.0 for balanced contribution.
        """
        random_vector = self.vectors_random()
        personal_vector = self.vectors_personal_best()
        global_vector = self.vectors_global()
        
        # Weighted combination
        stacked_vectors = np.stack([random_vector, personal_vector, global_vector], axis=1)
        particles_vector = np.zeros_like(self.particles)
        
        # Discrete node: weighted random choice
        weights = np.array([w_inertia, w_personal, w_global])
        for i in range(len(self.particles)):
            node_choices = stacked_vectors[i, :, 0].astype(int)
            particles_vector[i, 0] = np.random.choice(node_choices, p=weights)
        
        # Continuous parameters: weighted average
        particles_vector[:, 1:] = (
            w_inertia * stacked_vectors[:, 0, 1:] +
            w_personal * stacked_vectors[:, 1, 1:] +
            w_global * stacked_vectors[:, 2, 1:]
        )
        particles_vector[:, 1:] = np.clip(
            particles_vector[:, 1:], 
            self.param_bounds[0, :], 
            self.param_bounds[1, :]
        )
        ...
        swarm_proposed = ParticleSwarm(self.graph_instance)
        swarm_proposed.create_particles_from_array(particles_vector)
        proposed_signals = swarm_proposed.run_parallel(self.snirf_data)
    ...
    def run_cycle(self, max_steps=15, print_results=False, 
                               stopping_error_threshold=4, evaluation_data=None,
                               w_inertia=0.33, w_personal=0.33, w_global=0.34):
        """
        Run optimization using standard PSO instead of metric tensor approach.
        ...
        self.particle_personal_bests = self.particles.copy()
        self.particle_personal_errors = np.full(self.num_particles, np.inf)
        ...
        for i, error in enumerate(initial_errors):
            if error < self.particle_personal_errors[i]:
                self.particle_personal_errors[i] = error
                self.particle_personal_bests[i] = self.particles[i].copy()
        ...
        while not found_solution and step < max_steps:
            ...
            # Use standard PSO update instead of metric tensor
            movement_vector = self.total_vector_standard(w_inertia, w_personal, w_global)
        ...
        # Final evaluation (same as metric tensor version)
        ...
        meta = {
            "run_name": self.name_prefix + "_standard_pso",
            "method": "standard_pso",
            "pso_weights": {"inertia": w_inertia, "personal": w_personal, "global": w_global},
